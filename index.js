...
/* =========================
   SEND PRODUCT MESSAGE (helper compartido)
========================= */
...
async function sendCatalog(amount = CATALOG_BATCH) {
  ...
  while (sent < amount) {
    ...
    console.log(`Sending product ${sent + 1}/${amount}: ${p.nombre} [index ${state.catalogIndex}]`);
...
/* =========================
   AUTO-POST con rotación (FIX PRINCIPAL)
========================= */

async function autoPost(channel) {
  if (!products.length) return;

  // Filtramos solo productos con imagen
  ...
  // Tomamos AUTO_POST_BATCH productos desde el índice actual,
  // rotando sobre validIndexes
  const batch = [];
  ...
  if (cmd === "!help") {
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        "🔄 **`!reset`**\n" +
        "• Resetea el índice al producto 0\n\n" +
        "📊 **`!status`**\n" +
...
client.once("clientReady", async () => {
  ...
  // Intervalo de auto-post (por defecto cada 5 horas)
